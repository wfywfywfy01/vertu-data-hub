...
- 资产登记和原件导出携带 `Idempotency-Key`，每个请求携带 `X-Request-ID`。
- 列表当前最多返回 100 项；错误返回稳定 `code`、安全 `message` 和 `request_id`，不得返回密钥、SQL 或原始敏感内容。
...
| `GET` | `/v1/reviews` | 管理员获取高敏感资料待确认项 |
| `POST` | `/v1/reviews/{review_id}/decision` | 管理员批准重新处理或拒绝入库 |
| `GET/POST` | `/v1/dealers` | 查询或建议经销商主表记录 |
| `PATCH` | `/v1/dealers/{dealer_id}/confirm` | 管理员确认正式名称 |
| `POST` | `/v1/exports` | 管理员二次认证后申请一次性原件下载 |
| `GET` | `/v1/exports/{export_id}/download` | 在 5 分钟内使用一次性令牌下载原件 |
...
`asset_id`、至少 10 字的 `reason`、`confirmation=export-original`，并携带
`Idempotency-Key`。接口返回一次性 `download_url` 和 `download_token`。下载请求
通过 `X-Export-Token` 携带令牌，避免令牌进入 URL 和访问日志。原件字节保持不变；
下载授权 5 分钟过期且成功使用后立即失效。服务记录脱敏原因、原因哈希、版本、
敏感级别和字节数；非管理员即使知道资产 ID 也返回 403。

### 高敏感资料确认

文档、图片 OCR 和音视频转写在建立向量前检测密码、API 密钥、私钥、身份证号、
银行卡号、敏感文件名和 `restricted` 标记。命中后资产进入 `awaiting_review`，不生成
检索片段。仅 `admin` 可通过 `/v1/reviews` 查看并批准或拒绝：批准后原任务重新排队，
拒绝后资产逻辑删除；两种决定都写入追加式审计。
管理员可在决定前使用同一套二次认证、用途说明和一次性令牌下载待确认原件；普通
预览和检索仍拒绝 `awaiting_review` 资产。
...
| 角色 | 默认范围 | 能力 |
|---|---|---|
| `sales` | 已分配经销商 | 上传、查询、提出主表修改 |
| `manager` | 明确配置的团队 | 上传、查询团队资料 |
