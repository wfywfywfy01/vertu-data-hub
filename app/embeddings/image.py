...
class ApiImageEmbedder:
    ...
    def __init__(self) -> None:
        if not settings.image_embedding_api_key:
            raise RuntimeError(
                "IMAGE_EMBEDDING_API_KEY is empty; set it in .env or use IMAGE_EMBEDDING_PROVIDER=hash"
            )
...
class HashImageEmbedder:
    ...
    async def embed_image(self, data: bytes) -> list[float]:
        ...
        pixels = list(img.getdata())  # GRID*GRID 个 (r,g,b)
